...
const checkSpecNumber = (string) => {
  ...
  // 3. 十进制包含科学计数法 /^(\.[0-9]+|(?:[1-9]+[0-9]*|0)(?:\.[0-9]*|\.)?)(?:[eE][+-]{0,1}[0-9]+)?(?![_$a-zA-Z0-9])$/
  const reg$1 = /^([+-])?Infinity$/
  const reg$2 = /^([+-])?((0b|0B)([0-1]+)(?![0-1])|(0o|0O)([0-7]+)(?![0-7])|(0x|0X)([0-9a-f-A-F]+)(?![0-9a-f-A-F]))$/
  const reg$3 = /^([+-])?(\.[0-9]+|([1-9]+[0-9]*|0)(\.[0-9]*|\.)?)([eE][+-]{0,1}[0-9]+)?(?![_$a-zA-Z0-9])$/
...
const groupNormalize = (radix) => {
  if (radix === 10) {
    ...
    const int = RegExp.$3
    const fraction = RegExp.$4
    const e = RegExp.$5

    return { operator, int, fraction, e }
  }
  const operator = RegExp.$1
  ...
  return {
    operator, binaryPerfix, binary, octalPerfix, octal, hexPerfix, hex
  }
}
...
export const StringToNumber = (string) => {
  ...
  if (radix && (radix !== 10)) {
    ...
    const result = toNumberByOperator(toDecimal(binary || octal || hex, radix), operator)
    return new Number(result).valueOf()
  }
  ...
  if (radix && radix === 10) {
    ...
    if (e) {
      ...
      const result = toNumberByOperator(m ? ((int * 10 ** m + n * 1)) / (10 ** (m + Math.abs(exponent))) : int * 10 ** exponent, operator)
      return new Number(result).valueOf()
    }
    const result = toNumberByOperator(`${int}${fraction}`, operator)
    return new Number(result).valueOf()
  }
}
